...
func TestMaybeUpgradeIdleServerForControl(t *testing.T) {
	tests := []struct {
		name             string
		status           runningServerStatus
		upgradeResult    bool
		wantUpdated      bool
		wantUpgradeCalls int
	}{
		{
			name: "current server",
			status: runningServerStatus{
				version:      monkeyMuxVersion,
				capabilities: []string{"idle-upgrade-v1"},
			},
		},
		{
			name: "older server without atomic idle upgrade",
			status: runningServerStatus{
				version:      "0.1.0",
				capabilities: []string{"shutdown", "attach-state"},
			},
		},
		{
			name: "older busy server",
			status: runningServerStatus{
				version:      "0.1.0",
				capabilities: []string{"idle-upgrade-v1"},
			},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			upgradeCalls := 0
			updated, err := maybeUpgradeIdleServerForControl(
				"work",
				func(string) (runningServerStatus, error) {
					return test.status, nil
				},
				func() (bool, error) {
					upgradeCalls++
					return test.upgradeResult, nil
				},
			)
			if err != nil {
				t.Fatalf("maybeUpgradeIdleServerForControl returned error: %v", err)
			}
			if updated != test.wantUpdated {
				t.Fatalf("updated = %v, want %v", updated, test.wantUpdated)
			}
			if upgradeCalls != test.wantUpgradeCalls {
				t.Fatalf(
					"upgrade calls = %d, want %d",
					upgradeCalls,
					test.wantUpgradeCalls,
				)
			}
		})
	}
...
func TestMaybeUpgradeIdleServerForControlPropagatesUpgradeFailure(t *testing.T) {
	...
	if !errors.Is(err, upgradeErr) {
		t.Fatalf("error = %v, want %v", err, upgradeErr)
	}
}

func TestPrepareIdleUpgradeRejectsForegroundAttach(t *testing.T) {
	server := newMuxServer("work")
	server.attachConn = &recordingConn{}

	restore, claimed := server.prepareIdleUpgrade()

	if claimed {
		t.Fatal("idle upgrade claimed a server with a foreground attach")
	}
	if restore != nil {
		t.Fatal("rejected idle upgrade returned a restore snapshot")
	}
	if server.isUpgrading() {
		t.Fatal("rejected idle upgrade left the server draining")
	}
}

func TestPrepareIdleUpgradeClaimsServerOnce(t *testing.T) {
	server := newMuxServer("work")
	...
	if !claimed {
		t.Fatal("idle server was not claimed for upgrade")
	}
	if restore == nil || len(restore.Windows) != 1 {
		t.Fatalf("restore = %#v, want one window", restore)
	}
	if !server.isUpgrading() {
		t.Fatal("claimed server did not enter the upgrading state")
	}
	if _, claimedAgain := server.prepareIdleUpgrade(); claimedAgain {
		t.Fatal("server allowed a second idle upgrade claim")
	}
}

func TestHandleAttachRejectsConnectionDuringIdleUpgrade(t *testing.T) {
	server := newMuxServer("work")
	...
	server.handleAttach(
		...
		controlMessage{Role: "attach", Session: "work"},
	)

	if server.hasAttachClient() {
		t.Fatal("upgrading server accepted a foreground attach")
	}
	if err := clientConn.SetReadDeadline(time.Now().Add(time.Second)); err == nil {
		var data [1]byte
		if _, err := clientConn.Read(data[:]); err == nil {
			t.Fatal("rejected attach connection remained open")
		}
	}
}

func TestRequestIdleServerUpgradeClaimsServerOverControlSocket(t *testing.T) {
	runtimeBase, err := os.MkdirTemp("/tmp", "monkeymux-test-")
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
	...
	const session = "idle-upgrade-control"
	...
	if !claimed {
		t.Fatal("idle-upgrade control request did not claim the server")
	}
	if restore == nil || len(restore.Windows) != 1 {
		t.Fatalf("restore = %#v, want one window", restore)
	}
	...
	case <-time.After(time.Second):
		t.Fatal("idle-upgrade control connection did not close")
	}
	if !server.isClosed() {
		t.Fatal("idle-upgrade control request did not close the old server")
	}
}
