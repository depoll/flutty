...
func TestActiveContextRefreshesReplacementWindowWorkingDirectory(t *testing.T) {
	originalWorkingDirectory := processWorkingDirectoryForMetadata
	t.Cleanup(func() {
		processWorkingDirectoryForMetadata = originalWorkingDirectory
	})
	...
	processWorkingDirectoryForMetadata = func(pid int) string {
		if pid != 202 {
			t.Fatalf("working-directory pid = %d, want replacement pid 202", pid)
		}
		return "/work/current"
	}
	...
	currentPath, _, err := server.activeWindowContext(true)
	if err != nil {
		t.Fatal(err)
	}

	if currentPath != "/work/current" {
		t.Fatalf("active context path = %q, want /work/current", currentPath)
	}
	if server.windows[1].cwd != "/work/current" {
		t.Fatalf("replacement cached cwd = %q, want /work/current", server.windows[1].cwd)
	}
}

func TestActiveContextRetriesWhenActiveWindowChangesDuringCwdProbe(t *testing.T) {
	originalWorkingDirectory := processWorkingDirectoryForMetadata
	t.Cleanup(func() {
		processWorkingDirectoryForMetadata = originalWorkingDirectory
	})
	...
	processWorkingDirectoryForMetadata = func(pid int) string {
		...
		case 101:
			...
			return "/work/wrong"
		case 202:
			return "/work/two-current"
		default:
			t.Fatalf("unexpected working-directory pid %d", pid)
			return ""
		}
	}
	...
	if currentPath != "/work/two-current" {
		t.Fatalf("active context path = %q, want /work/two-current", currentPath)
	}
	if server.windows[0].cwd != "/work/one" {
		t.Fatalf("stale probe overwrote old window cwd: %q", server.windows[0].cwd)
	}
	if server.windows[1].cwd != "/work/two-current" {
		t.Fatalf("active cached cwd = %q, want /work/two-current", server.windows[1].cwd)
	}
}

func TestActiveContextRetriesWhenForegroundProcessChangesDuringCwdProbe(
	t *testing.T,
) {
	originalWorkingDirectory := processWorkingDirectoryForMetadata
	t.Cleanup(func() {
	...
	processWorkingDirectoryForMetadata = func(pid int) string {
		...
		case 101:
			server.mu.Lock()
			server.windows[0].foregroundPid = 202
			server.mu.Unlock()
			return "/work/wrong"
		case 202:
			return "/work/current"
	...
	if server.windows[0].cwd != "/work/current" {
		t.Fatalf("active cached cwd = %q, want /work/current", server.windows[0].cwd)
	}
}

func TestActiveContextKeepsCachedPathWhenCwdProbeIsUnavailable(t *testing.T) {
	originalWorkingDirectory := processWorkingDirectoryForMetadata
	t.Cleanup(func() {
		processWorkingDirectoryForMetadata = originalWorkingDirectory
	})
	...
	if currentPath != "/work/cached" {
		t.Fatalf("active context path = %q, want cached path", currentPath)
	}
}

func TestActiveContextSkipsCwdProbeUnlessRequested(t *testing.T) {
	originalWorkingDirectory := processWorkingDirectoryForMetadata
	t.Cleanup(func() {
		processWorkingDirectoryForMetadata = originalWorkingDirectory
	})
	...
	processWorkingDirectoryForMetadata = func(int) string {
		t.Fatal("cache-only active context unexpectedly probed the cwd")
		return ""
	}

	currentPath, _, err := server.activeWindowContext(false)
	if err != nil {
		t.Fatal(err)
	}
