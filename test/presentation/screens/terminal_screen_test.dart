...
void main() {
  ...
  group('terminal theme RGB aliases', () {
    int rgbCell(int rgb) => CellColor.rgb | rgb;

    test('remaps Copilot RGB cells from MonkeySSH light to dark', () {
      final terminal = Terminal(maxLines: 100)
        ..write(
          '\x1b[38;2;36;62;59mE'
          '\x1b[38;2;91;92;156mM'
          '\x1b[39m\x1b[48;2;235;243;242mB',
        );
      final aliases = buildTerminalThemeRgbColorAliases(
        previousTheme: monkey_themes.TerminalThemes.defaultLightTheme,
        nextTheme: monkey_themes.TerminalThemes.defaultDarkTheme,
      );

      final changedCells = remapTerminalThemeRgbColorsInBuffer(
        terminal,
        aliases,
      );
      final line = terminal.buffer.lines[0];

      expect(changedCells, 3);
      expect(line.getForeground(0), rgbCell(0xBECDCB));
      expect(line.getForeground(1), rgbCell(0xA99AD8));
      expect(line.getBackground(2), rgbCell(0x0D1A20));
    });

    test('leaves already-readable dark RGB cells unchanged', () {
      final terminal = Terminal(maxLines: 100)
        ..write('\x1b[38;2;190;205;203mE');
      final aliases = buildTerminalThemeRgbColorAliases(
        previousTheme: monkey_themes.TerminalThemes.defaultLightTheme,
        nextTheme: monkey_themes.TerminalThemes.defaultDarkTheme,
      );
      ...
      expect(changedCells, 0);
      expect(terminal.buffer.lines[0].getForeground(0), rgbCell(0xBECDCB));
    });

    test('leaves ambiguous source RGB cells unchanged', () {
      final terminal = Terminal(maxLines: 100)..write('\x1b[38;2;13;43;40mF');
      final aliases = buildTerminalThemeRgbColorAliases(
        previousTheme: monkey_themes.TerminalThemes.defaultLightTheme,
        nextTheme: monkey_themes.TerminalThemes.defaultDarkTheme,
      );
      ...
      expect(changedCells, 0);
      expect(terminal.buffer.lines[0].getForeground(0), rgbCell(0x0D2B28));
    });

    test('leaves palette and default cells unchanged', () {
      final terminal = Terminal(maxLines: 100)..write('\x1b[32mG\x1b[39mD');
      final aliases = buildTerminalThemeRgbColorAliases(
        previousTheme: monkey_themes.TerminalThemes.defaultLightTheme,
        nextTheme: monkey_themes.TerminalThemes.defaultDarkTheme,
      );
      ...
      expect(changedCells, 0);
      expect(line.getForeground(0) & CellColor.typeMask, isNot(CellColor.rgb));
