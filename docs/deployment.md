...
Store screenshots can be regenerated locally with `python3 scripts/generate_store_screenshots.py` after installing Pillow (`python3 -m pip install Pillow`). The generator starts a temporary local `sshd` and `tmux` workspace, launches the real Claude Code and GitHub Copilot CLI TUIs on their streamer-safe starting screens (`CLAUDE_CODE_HIDE_ACCOUNT_INFO`/`CLAUDE_CODE_HIDE_CWD` with a clean demo home for Claude and `/streamer` for Copilot), boots the normal MonkeySSH app on iOS simulators and an Android emulator with release-demo data, drives real app navigation into the live SSH/tmux terminal, then captures native device screenshots into the Fastlane folders. The generator fails instead of substituting scripted terminal output if `claude`, `copilot`, or `tmux` are unavailable.
