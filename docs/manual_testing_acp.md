...
| Remote OS | Copilot CLI (`copilot --acp --no-color --no-auto-update --log-level error`) | OpenCode (`opencode acp --log-level ERROR`) | Pi (`pi --mode rpc`) | Expected differences to record |
| --- | --- | --- | --- | --- |
| macOS | Initialize, login handoff, new/list/load/resume, text/image/resource, permission, cancel, reconnect | Initialize, `opencode auth login`, new/list/load/resume, text/image/resource, permission, cancel, reconnect | Detect installed `pi`; test text/image/thinking/tool output, slash completion, model and reasoning controls, usage, cancel, durable reconnect, and extension dialog cancellation | Homebrew/npm PATH in SSH exec shells; provider-specific slash commands, modes/models, session titles, and permission wording |
| Linux | Same flow; test package and npm installs | Same flow; test installer and npm installs | Same flow; verify the Pi executable and RPC mode are available on PATH | Distribution shell/PATH, sandbox/tool availability, notification behavior when the phone backgrounds |
| Windows | Test native OpenSSH + PowerShell; repeat in WSL when used | Test native OpenSSH + PowerShell; repeat in WSL when used | Test the Pi executable and RPC mode in native Windows and WSL separately | Native path/quoting and executable suffixes versus POSIX paths in WSL; some versions may advertise different load/resume, image, or embedded-resource capabilities |
