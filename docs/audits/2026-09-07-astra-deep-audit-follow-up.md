...
- Painter/cache teardown explicitly releases native paragraphs rather than waiting for garbage collection. Cache capacity and Quiver hit/eviction policy are unchanged.
...
| `flutter test --no-pub --reporter expanded` | 4,106 passed; one opt-in localhost SSH integration test skipped |
...
- Future ACP requests for an individually closed session on a still-shared bridge need an explicit session-reactivation policy. This patch invalidates requests already in progress; it does not add a permanent session-ID denylist that could break later resume.
