...
Regenerate repository-managed screenshots with scripts/generate_store_screenshots.py. The screenshots must be captured from the normal MonkeySSH app running on simulators/emulators with seeded release-demo data and a live temporary local SSH/tmux workspace, not individual screens mounted directly, scripted terminal transcripts, or synthetic mockups. Favor terminal, tmux, Claude Code, Copilot, snippets, and agent-workspace flows; do not use subscription or checkout screens as store screenshots. Claude Code and Copilot screenshots must show the real interactive CLI interfaces running inside tmux on their streamer-safe starting screens, using real privacy controls (`CLAUDE_CODE_HIDE_ACCOUNT_INFO`/`CLAUDE_CODE_HIDE_CWD` with a clean demo home for Claude and `/streamer` for Copilot) that hide emails, usernames, tokens, hostnames, and private identifiers. If a real CLI cannot be launched safely, stop and ask before substituting any fallback. Keep iPhone and iPad screenshots under ios/fastlane/screenshots/en-US. Keep Android phone, 7-inch tablet, and 10-inch tablet screenshots under android/fastlane/metadata-*/android/en-US/images/{phoneScreenshots,sevenInchScreenshots,tenInchScreenshots}. Make sure Fastlane continues to upload metadata, images, and screenshots from the repository.
