...
- **PR Preview Builds**: Every PR automatically builds a `private` preview and posts a downloadable APK. Add a 🚀 reaction to the sticky preview comment to queue the current preview SHA for TestFlight + Play internal deployment (the scanner checks every 5 minutes), or run `Deploy PR Preview` manually from the Actions tab.
